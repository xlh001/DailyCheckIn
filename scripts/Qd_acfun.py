...
class AcFun:
    ...
    def login(phone, password, session):
        ...
        return (True, res) if res.get("result") == 0 else (False,
                                                           res.get("err_msg"))
    ...
    def get_cookies(session, phone, password):
        ...
        headers = {
            ...
            "devicetype": "0",
            "accept-language":
            "zh-Hans-CN;q=1, en-CN;q=0.9, ja-CN;q=0.8, zh-Hant-HK;q=0.7, io-Latn-CN;q=0.6",
    ...
    def get_token(self, session):
        ...
        self.st = res.get("acfun.midground.api_st") if res.get(
            "result") == 0 else ""
        return self.st
    ...
    def danmu(self, session):
        ...
        subChannel = re.findall(
            r'{subChannelId:(\d+),subChannelName:"([\u4e00-\u9fa5]+)"}',
            response.text)
        if videoId:
...
def acfun():
    ...
    if env_spl(_datas) != None:
        for check_item in env_spl(_datas):
            _check_item={"phone": check_item[0], "password": check_item[1]}
